...
const createTouchTexture = (): TouchTextureHandle => {
  const size = 64;
  ...
  const drawPoint = (p: TouchPoint): void => {
  ...
  const addTouch = (norm: { x: number; y: number }): void => {
    let force = 0;
    let vx = 0;
    let vy = 0;
    if (last) {
...
export function PixelBlast({
  ...
  useEffect(() => {
    ...
    if (!threeRef.current) mustReinit = true;
      ...
      if (
        prev.antialias !== cfg.antialias ||
        prev.liquid !== cfg.liquid ||
        prev.noiseAmount !== cfg.noiseAmount
      ) {
        mustReinit = true;
      }
    }
    ...
    if (mustReinit) {
      ...
      const setSize = (): void => {
        ...
        const res = uniforms.uResolution!.value as THREE.Vector2;
      ...
      const onPointerDown = (e: PointerEvent): void => {
        ...
        const pos = uniforms.uClickPos!.value as THREE.Vector2[];
        pos[ix]!.set(fx, fy);
        const times = uniforms.uClickTimes!.value as Float32Array;
        times[ix] = uniforms.uTime!.value as number;
        if (threeRef.current) {
