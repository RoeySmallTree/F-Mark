 ...
 * design document. This module PATCHes the applied theme to `PATCH /theme` on
 * boot and on every change, so the tool reflects what the user actually sees.
 *
...
function postTheme(name: ThemeName): void {
...
/**
 * Start reporting the active theme to the kernel: one immediate report plus a
 * subscription for subsequent changes. Returns an unsubscribe function.
 */
export function startThemeReporting(): () => void {
  postTheme(getCurrentTheme());
