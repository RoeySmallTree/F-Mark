...
- Use the \`fmark://guide\` resource when you need this guide again; \`fmark://theme\` returns the active theme + font design tokens.
...
- **fmark-ui** — UI that will ship in F-Mark itself (a new panel, a redesigned list, a settings screen — anything that becomes part of the F-Mark app). The renderer source is the authority: (a) locate the real component in \`packages/renderer/src/\` (start from \`panels/sessions/SessionsPanelView.tsx\`, \`shell/shell.css\`, \`themes/tokens.css\`, \`themes/fonts.css\`); (b) reuse its actual class names and structural CSS verbatim; (c) resolve colors and active font stacks via \`fmark_get_theme\` tokens / \`var(--…)\` so the mockup inherits whatever appearance is applied. Identify the source tokens first; resolve specific theme values only after. Treat the design document only as a token reference, never as a layout or typography guide.
