...
```json
{ "kind": "agent", "name": "Opencode", "suggested_id": "ag-opencode" }
```
...
```json
{
  ...
  "status": "open",
  "assigned_to": "ag-opencode",
  "parent_id": "td-launch-plan",
  "supersedes": "20260522T143512Z_ag-opencode.todo.json"
}
```
...
Sets the active session for a participant. Linking is expected so the renderer, plugin, and other agents know where the Opencode agent is currently active.
...
**Response (200):**
```json
{ "participant_id": "ag-opencode", "session_id": "2026-05-22-launch-plan" }
```
...
Logs a tool invocation. In managed Opencode sessions the plugin posts this automatically after each tool call returns.
...
When set to `true`, the renderer groups the message into the collapsible mid-turn box. The Opencode plugin sets `arbitrary: true` on intermediate text and leaves the final concluding text as normal prose.
